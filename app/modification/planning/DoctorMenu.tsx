...
export const DoctorMenu = memo(
  ({
    ...
    useEffect(() => {
      console.log('loadExceptionHours called with:', { shiftId, doctorId: doctor?.id });
    ...
    const saveExceptionHours = async () => {
      ...
      try {
        ...
        {
          setExceptionHours(tempExceptionHours > 0 ? tempExceptionHours : null);
          setShowExceptionInput(false);
          toast.success(tempExceptionHours > 0 ? 'Exception horaire ajoutée' : 'Exception horaire supprimée');
          console.log('Calling onUpdateException:', { doctorId: doctor.id, hours: tempExceptionHours });
          if (onUpdateException) {
            onUpdateException(doctor.id, tempExceptionHours > 0 ? tempExceptionHours : null);
          }
        }
      } catch (error) {
    ...
    const handleIncreaseShare = () => {
      if (canIncrease) {
        ...
        console.log('Increasing share:', {
          doctorId: doctor?.id,
          isMaintenance: assignment.maintenance,
          isNoDoctor: assignment.noDoctor,
          newShare: localAssignment.share + 1,
        });
        onIncreaseShare();
      }
    };
