...
import { getWeekDays, formatFrenchDate, formatDateKey, getWeekNumber, getDateOfWeek } from './accueil/utils';
import { useDoctors } from './accueil/hooks/useDoctors';
...
export default function Home() {
  ...
  // Charger les données initiales pour la semaine en cours
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        // Utiliser la semaine en cours
        const { week: todayWeek, year: todayYear } = getWeekNumber(today);
        const weekDate = getDateOfWeek(todayWeek, todayYear);

        setCurrentDate(weekDate);
        setSelectedWeek(todayWeek);
        setSelectedYear(todayYear);

        const gardesData = await getGardesForYear(todayYear);
        setGardes(gardesData);
        const congesData = await getCongesForWeek(todayYear, todayWeek);
  ...
  // Charger les affectations pour la semaine en cours
  useEffect(() => {
    ...
    <div className="p-0 max-w-full mx-0 no-outer-margins">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-2 p-2 bg-gray-50 rounded-lg">
        <h1 className="text-xl font-bold">
          Planning de la semaine {selectedWeek} ({currentDate ? getWeekDays(currentDate)[0]?.toLocaleDateString('fr-FR') : 'N/A'} au{' '}
          {currentDate ? getWeekDays(currentDate)[5]?.toLocaleDateString('fr-FR') : 'N/A'})
        </h1>
